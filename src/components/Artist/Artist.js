...
const Artist = (props) => {
    ...
    return (
        <div className="col-md-4 col-lg-4 text-center">
            <div className="bg-white my-3 pb-4 rounded-3 ">
            <div>
                <img className="rounded-circle w-50 py-3" src={img} alt="" />
            </div>
            <div className="text-center px-3 fw-bold cart">
                <p>Name: {name}</p>
                <p>Born: {born}</p>
                <p>Known-For: {knownFor}</p>
                <p>Salary:${salary}</p>
                {
                    props.artist.isAdded ? (
                    <button className="btn btn-primary">Already Added</button>) :
